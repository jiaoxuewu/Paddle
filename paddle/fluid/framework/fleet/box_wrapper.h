...
class BoxWrapper {
 public:
  ...
  static std::shared_ptr<BoxWrapper> GetInstance() {
	...
	VLOG(3) << "Enter for";
	// VLOG(3) << "CUDA_VISIBLE_DEVICES[" << std::getenv("CUDA_VISIBLE_DEVICES") << "]";
        for (int i = 0; i < platform::GetCUDADeviceCount(); ++i) {
	  VLOG(3) << "before get context i[" << i << "]";
          platform::CUDADeviceContext* context = dynamic_cast<platform::CUDADeviceContext*>(
	...
	VLOG(3) << "before call InitializeGPU";
        s_instance_->boxps_ptr_.reset(new boxps::PaddleBoxPS());
	s_instance_->boxps_ptr_->InitializeGPU("./conf/db.yaml", 32, stream_list);
